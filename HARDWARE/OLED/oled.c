...
{      			    
    ...
    for(t=0;t<16;t++)
	{
		OLED_WR_Byte(qianrushi[no][t],OLED_DATA);
		addr+=1;
    }	
	OLED_Set_Pos(x,y+1);	
    for(t=16;t<32;t++)
...
void OLED_Showtitle(void)
{
	...
	OLED_ShowString(80,1,"15",50);
	OLED_ShowCHinese(100,0,6);	
}

//显示MQ2
void OLED_ShowMQ2(u8 x,u8 y,u8 no)
{      			    
	u8 t,addr=0;
	OLED_Set_Pos(x,y);	
    for(t=0;t<16;t++)
	{
		OLED_WR_Byte(yanwu[no][t],OLED_DATA);
		addr+=1;
    }	
	OLED_Set_Pos(x,y+1);	
    for(t=16;t<32;t++)
...
//显示MQ135
void OLED_ShowMQ135(u8 x,u8 y,u8 no)
{      			    
	u8 t,addr=0;
	OLED_Set_Pos(x,y);	
    for(t=0;t<16;t++)
	{
		OLED_WR_Byte(kongqi[no][t],OLED_DATA);
		addr+=1;
    }	
	OLED_Set_Pos(x,y+1);	
    for(t=16;t<32;t++)
